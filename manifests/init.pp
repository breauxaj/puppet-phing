...
class phing (
  ...
  exec { 'get-phing':
    ...
    command => "wget ${source}",
    cwd     => '/usr/local/bin',
    creates => '/usr/local/bin/phing-latest.phar',
    timeout => 10000,
    onlyif  => "test ! -f /usr/local/bin/phing-latest.phar",
  }

  file { '/usr/local/bin/phing-latest.phar':
    ensure  => present,
    owner   => 'root',
    group   => 'root',
    mode    => '0755',
  ...
  file { '/usr/local/bin/phing':
    ...
    owner   => 'root',
    group   => 'root',
    target  => "/usr/local/bin/phing-latest.phar",
    require => Exec['get-phing'],
  }
